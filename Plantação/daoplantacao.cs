...
namespace FarmSystem.Plantação
{
    class daoplantacao
    {
        ...
        {
            Conexao conn = new Conexao();
            NpgsqlCommand query = new NpgsqlCommand("Select codigo,funcionario,area, distanciaentreplantas, datadeplantio," +
           "valordevenda,statusdaplantacao,previsaodecolheita, prevdatacolheita, quantidadecolhida,sementeusada" +
                  "from farmsystem.plantacao where LIKE @semente");
            ...
            using (NpgsqlDataReader dr = query.ExecuteReader())
            {
                if (dr.HasRows)
                {
                     ...
                     */
                        ...
                        p.codigo = dr.GetInt32(0);
                        p.funcionario = dr.GetInt32(1);
                        p.area = dr.GetFloat(2);
                        p.distinciaentreplantas = dr.GetFloat(3);
                        p.datadeplantio = dr.GetDateTime(4);
                        p.valordevenda = dr.GetString(5);
                        p.statusdaplantacao = dr.GetString(6);
                        p.previsaodecolheita = dr.GetInt32(7);
